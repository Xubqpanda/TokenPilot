...
REPO_ROOT="$(cd "${PINCHBENCH_ROOT}/../.." && pwd)"
TOKENPILOT_EXPERIMENT_ROOT="${REPO_ROOT}"
# shellcheck source=../../../runtime/resolve-runtime.sh
source "${TOKENPILOT_EXPERIMENT_ROOT}/runtime/resolve-runtime.sh"
# shellcheck source=../../../shared/shell/profile.sh
source "${TOKENPILOT_EXPERIMENT_ROOT}/shared/shell/profile.sh"
...
load_benchmark_profile() {
  ...
  local mode="${2:?session mode is required}"
...
require_method_runtime_env() {
  if [[ -z "${TOKENPILOT_BASE_URL:-}" ]]; then
    printf 'Missing TOKENPILOT_BASE_URL. Define it in benchmarks/pinchbench/.env or the shell environment.\n' >&2
    return 1
  fi
  if [[ -z "${TOKENPILOT_API_KEY:-}" ]]; then
    printf 'Missing TOKENPILOT_API_KEY. Define it in benchmarks/pinchbench/.env or the shell environment.\n' >&2
    return 1
  fi
}
