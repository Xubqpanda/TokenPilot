...
This document narrows the benchmark merge scope for the first real consolidation pass.

The goal is not to import the entire external benchmark repository. The goal is to extract the minimum coherent PinchBench experiment surface that we actively use.
...
These are the first files and directories worth consolidating into the main repository:
...
- they may still contain historical assumptions inherited from the external
  harness, but the active migrated path has already been reduced to single-agent PinchBench execution
