...
This document defines the target layout for the PinchBench experiment subtree inside the main repository.

The goal is not to import the external benchmark repository wholesale. The goal is to create a clean, maintainable experiment surface for the parts we still actively use.
...
### `benchmarks/pinchbench/save/`

This directory should hold curated saved experiment artifacts that remain worth referencing.
...
Method names should appear only where method-specific behavior genuinely exists.
